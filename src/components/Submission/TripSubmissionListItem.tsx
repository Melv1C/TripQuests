...
export function TripSubmissionListItem(props: TripSubmissionListItemProps) {
  const { submission, questTitle, questPoints } = props;
  ...
  const formattedReviewTime = React.useMemo(() => {
    ...
    try {
      ...
      if (submission.reviewedAt instanceof Timestamp) {
        return formatDistanceToNow(submission.reviewedAt.toDate(), { addSuffix: true });
      }
      // Handle regular Date objects
      else if (submission.reviewedAt instanceof Date) {
        return formatDistanceToNow(submission.reviewedAt, { addSuffix: true });
      }
      // Handle string or number timestamps by converting to Date
      return formatDistanceToNow(new Date(submission.reviewedAt), { addSuffix: true });
    } catch (error) {
      console.error('Error formatting date:', error);
      return null;
    }
    ...
    <Paper variant="outlined" sx={{ mb: 1, overflow: 'hidden' }}>
      <Box sx={{ p: 2 }}>
        <Grid container spacing={2} alignItems="flex-start">
          ...
          <Grid size={{ xs: 12, sm: submission.evidence.imageUrl ? 8 : 12 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
                <Avatar sx={{ width: 28, height: 28, mr: 1 }}>
                  {submission.submitterPseudo.charAt(0).toUpperCase()}
                </Avatar>
                <Box>
                  ...
                  {submission.status !== 'pending' && formattedReviewTime && (
                    <Typography variant="caption" color="text.secondary" display="block">
